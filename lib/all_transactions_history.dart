...
class WalletTransactionItem {
  ...
  factory WalletTransactionItem.fromMap(
    ...
    return WalletTransactionItem(
      ...
      createdAt: createdAt,
      statusLabel: _formatStatusStatic(rawStatus),
      statusKey: rawStatus.toLowerCase(),
      icon:
          _iconForChannelStatic(serviceType.isNotEmpty ? serviceType : channel),
      isIncoming: isIncoming,
      reference: reference,
      channelLabel: serviceType.isNotEmpty
          ? _titleCaseStatic(serviceType)
          : (channel.isEmpty ? 'Wallet' : _titleCaseStatic(channel)),
      metadata: metadata,
    );
  }
  ...
  static IconData _iconForChannelStatic(String channel) {
  ...
  void _addServiceSpecificDetails(String serviceType,
    ...
    // Add timestamp from metadata for all services
    final timestamp = metadata['timestamp']?.toString();
    if (timestamp != null && timestamp.isNotEmpty) {
      try {
        final dt = DateTime.parse(timestamp).toLocal();
