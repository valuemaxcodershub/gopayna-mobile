...
/// Fetch electricity bill for postpaid meters
Future<Map<String, dynamic>> fetchElectricityBill(
    String token, String disco, String meterNumber, String meterType) async {
  try {
    final response = await http.post(
      Uri.parse('$vtuBaseUrl/fetch-bill'),
      headers: _authorizedJsonHeaders(token),
      body: jsonEncode({
        'disco': disco,
    ...
    log('Fetch electricity bill failed: $e', name: 'api_service');
    return {'error': 'Unable to fetch electricity bill. Please try again.'};
  }
}
...
Future<Map<String, dynamic>> buyAirtime(
  ...
  try {
    ...
    if (bonusType != null) {
      requestBody['bonusType'] = bonusType;
    }
