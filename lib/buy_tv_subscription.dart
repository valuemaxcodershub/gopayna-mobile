...
class _BuyTVSubscriptionScreenState extends State<BuyTVSubscriptionScreen>
  ...
  String? _token;
  String _tvPackagesEmptyMessage = 'No synced TV packages available';
    ...
    try {
        ...
        if (packages.isNotEmpty) {
          ...
          setState(() {
            ...
            if (!_packages.containsKey(_selectedProvider)) {
              _selectedProvider =
                  _packages.keys.isNotEmpty ? _packages.keys.first : 'dstv';
            }
            _selectedPackage = '';
            _tvPackagesEmptyMessage = 'No synced TV packages available';
          });
          return;
        }
      }
      ...
      if (mounted) {
        setState(() {
          _packages.clear();
          _selectedPackage = '';
          _tvPackagesEmptyMessage =
              'Unable to load synced TV packages right now';
        });
      }
    }
  }
  ...
  void _buyTVSubscription() {
    if (_formKey.currentState!.validate()) {
      if (_selectedPackage.isEmpty || _currentPackages.isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
  ...
  void _processPurchase() async {
    ...
    try {
      selectedPackage = _currentPackages.firstWhere(
        (package) =>
            '${package['bundle']} - ${package['duration']}' == _selectedPackage,
      );
    } catch (e) {
              ...
              children: [
                ...
                Container(
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(
                        color:
                            _isSmartCardVerified ? Colors.green : borderColor),
                    boxShadow: isDarkMode
                        ? null
                        : [
                            BoxShadow(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      AnimatedSize(
                        ...
                        child: _showPackageList && _currentPackages.isNotEmpty
                            ? Container(
                                margin:
