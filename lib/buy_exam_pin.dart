...
class _BuyEducationPinScreenState extends State<BuyEducationPinScreen>
  ...
  String? _token;
  String _examPricingEmptyMessage = 'No synced exam services available';
    ...
    try {
        ...
        if (data.isNotEmpty) {
          ...
          setState(() {
            if (grouped.isNotEmpty) {
              ...
              _examPricingEmptyMessage = 'No synced exam services available';
            } else {
              _packages = {};
              _selectedProvider = '';
              _selectedPackage = '';
      ...
      if (mounted) {
        setState(() {
          ...
          _selectedProvider = '';
          _selectedPackage = '';
          _examPricingEmptyMessage =
              'Unable to load synced exam services right now';
        });
      }
    } finally {
  ...
  Future<void> _loadRecentTransactions() async {
    ...
    final result =
        await api.fetchVTUHistory(token, type: 'education', limit: 8);
    if (mounted && result['success'] == true) {
      final data = result['data'] as List? ?? [];
      setState(() {
        _recentTransactions = data.map((tx) {
          ...
          return EducationTransaction(
            ...
            date: DateTime.tryParse(tx['createdAt']?.toString() ?? '') ??
                DateTime.now(),
            status: _educationRecentListStatusLabel(tx['status']),
                        ...
                        Container(
                          ...
                          child: Center(
                            child: Text(
                              _providers.isEmpty
                                  ? _examPricingEmptyMessage
                                  : 'No synced packages available for this provider.',
                              textAlign: TextAlign.center,
                              style: TextStyle(
                                color: mutedTextColor,
                                fontSize: 14,
                        ...
                        Container(
                          ...
                          child: GridView.builder(
                            ...
                            gridDelegate:
                              ...
                              mainAxisSpacing: 6,
                              childAspectRatio:
                                  2.0, // Adjusted for longer text like "WAEC Result Checker"
                            ),
